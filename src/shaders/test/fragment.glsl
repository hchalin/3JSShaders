...
void main()
{
    ...
    // Pattern 23  (RANDOM STATIC NOISE)
    // float strength = random(vUv);

    // Pattern 24  (random grid squares)
    // vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor(vUv.y * 10.0) / 10.0);

    // float strength = random(gridUv);

    // Pattern 25 (tilted random grid squares)
    // vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor((vUv.y + vUv.x * 0.5) * 10.0) / 10.0);

    // float strength = random(gridUv);

    // Pattern 26
    // float strength = length(vUv.x);

    // Pattern 27
    // float strength = distance(vUv, vec2(0.5));

    // Pattern 28 (oppositie of above)
    // float strength = 1.0 - distance(vUv, vec2(0.5));

    // Pattern 29 (lense flare)
    ...
    // float strength = step(0.01,abs(distance(vUv, vec2(.5)) - .25));

    // Pattern 36
    //  float strength = 1.0 - step(0.01,abs(distance(vUv, vec2(.5)) - .25));
    ...
    // );
    // float strength = 1.0 - step(0.01,abs(distance(wavedUv, vec2(.5)) - .25));
